...
export function listToolPolicies(platform?: string, userId?: string): ToolPolicy[] {
	const db = initSecurityStore();
	ensureTable(db);
...
/**
 * Forget that the `tool_policies` table was already created (test-only helper).
 *
 * Must be called after `resetSecurityStore()` swaps in a fresh database, so the
 * next operation re-runs `CREATE TABLE IF NOT EXISTS` on the new DB instead of
 * assuming the table already exists from a previous (now-closed) connection.
 */
...
function getExplicitPolicies(db: Database.Database): ToolPolicy[] {
	try {
		const rows = db.prepare("SELECT * FROM tool_policies").all() as ToolPolicyRow[];
		return rows.map(rowToPolicy);
	} catch {
		return [];
	}
 ...
 */
export function isToolAllowed(platform: string, userId: string, toolName: string): boolean {
	...
	const matching = allPolicies.filter(
		(p) =>
			...
			// A policy only applies to the requesting user when its targeting
			// (platform / userId) matches — null acts as a wildcard. Without this
			// check, a policy scoped to one user would leak to everyone on the
			// platform (and could override the secure default deny).
...
export function getEffectivePolicySummary(_platform: string, _userId: string): EffectivePolicy {
	const db = initSecurityStore();
	ensureTable(db);
