...
switch (cmd) {
	case "start": {
		...
		if (running && pid !== undefined) {
			const existingHealth = await waitForGatewayHealth(loadHealthConfig(), pid, 1500);
			if (existingHealth) {
				console.log(
		...
		const startedHealth = await waitForGatewayHealth(loadHealthConfig(), child.pid, 5000);
		if (!startedHealth) {
			console.error(
	...
	case "stop": {
		...
		const verifiedHealth = await waitForGatewayHealth(loadHealthConfig(), pid!, 1500);
		if (!verifiedHealth) {
			console.error(
