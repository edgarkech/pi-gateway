...
export interface PlatformMessage {
	...
	channelId: string;
	userId: string;
	/** Textinhalt — DARF LEER sein, wenn die Nachricht nur Anhänge hat (Phase 3). */
	content: string;
	timestamp: number;
	metadata?: Record<string, unknown>;
	/**
	 * Phase 3 (File-Attachments): optional — lokal gespeicherte, validierte
	 * Anhänge. Backward-kompatibel: fehlt er, fließt die Nachricht unverändert
	 * durch die bestehende Pipeline (concept §5.2).
	 */
...
export interface PlatformAdapter {
	 ...
	 */
	sendInteractive(channelId: string, prompt: InteractivePrompt): Promise<{ messageId: string }>;
...
export abstract class BaseAdapter implements PlatformAdapter {
	...
	async cleanupInteractive(_channelId: string, _messageId: string): Promise<void> {
