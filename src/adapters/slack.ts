...
/** Incoming Slack event envelope passed to handleIncomingEvent(). */
...
export class SlackAdapter extends BaseAdapter {
	...
	constructor(config: SlackConfig) {
		super();
		// SlackConfig already requires `enabled` and `platform` (inherited from
	...
	async handleIncomingEvent(event: SlackIncomingEvent): Promise<void> {
		if (
			event.type === "message" &&
			!event.subtype &&
			event.channel &&
