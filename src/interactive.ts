...
export async function handleExtensionUiRequest(
	...
	try {
		const result = await adapter.sendInteractive(activeChannel.channelId, prompt);
		if (!result?.messageId) {
			logger.error(
...
export function handleInteractiveResponse(response: InteractiveResponse): void {
	let pending = response.requestId ? pendingUiRequests.get(response.requestId) : undefined;
...
function sendUiResponse(requestId: string, response: InteractiveResponse): void {
