...
// or programmatically through `initSessionStore(dir?)` / `resetSessionStore()`.

/** Resolve the directory that holds the session database file. */
 ...
 * the existing singleton. Use `resetSessionStore()` (tests only) to switch
 * to a fresh database.
 */
export function initSessionStore(dir?: string): Database.Database {
	if (db) return db;
	...
	db = new Database(join(gatewayDir, "gateway-sessions.db"));
  ...
  `);
	...
	db.exec(`CREATE INDEX IF NOT EXISTS idx_sessions_activity ON sessions(last_activity)`);
 ...
 * `initSessionStore(...)` call creates a brand-new database. Never call
...
export function resetSessionStore(): void {
	if (db) {
		try {
			...
			logger.warn("[SessionStore] Error closing test DB:", err);
		}
		db = null;
	}
}
  ...
  `,
	...
	if (existing) {
		...
		if (shouldResetSession(existing)) {
			...
			// Update last activity and return the refreshed session. The local
			// `existing` row was read before the write, so also refresh its
			// last_activity field to avoid returning a stale timestamp.
			existing.last_activity = Date.now();
			database
				.prepare("UPDATE sessions SET last_activity = ? WHERE id = ?")
				.run(existing.last_activity, existing.id);
...
function shouldResetSession(row: SessionRow): boolean {
	...
	if (now - row.last_activity > idleMs) {
		logger.info(`[SessionStore] Session ${row.id.slice(0, 8)} reset: idle timeout`);
		return true;
	}
...
export function cleanupStaleSessions(): number {
	...
	const result = database.prepare("DELETE FROM sessions WHERE last_activity < ?").run(cutoff);
...
// List active (non-background) channels across all platforms.
// Extracted from stopGatewayServer() so it can send shutdown notices.
export function listActiveChannels(): Array<{ platform: string; channelId: string }> {
	const database = initSessionStore();
	const rows = database
		.prepare("SELECT DISTINCT platform, channel_id FROM sessions WHERE is_background = 0")
