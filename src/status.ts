...
export type DetachedGatewayState = "stopped" | "healthy" | "initializing" | "unavailable";
...
export interface GatewayStatusReport {
	status: string;
	running: boolean;
	mode: "Inline" | "Detached" | "Detached initializing" | "Detached unavailable" | "Stopped";
...
export function normalizeGatewayHealthConfig(value: unknown): GatewayHealthConfig | null {
...
export async function fetchGatewayHealth(
	...
	try {
		const response = await fetchStatus(buildGatewayHealthUrl(config.host, config.port), {
			headers:
