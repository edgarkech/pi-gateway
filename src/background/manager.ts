...
export function initBackgroundTasks(): Database.Database {
  ...
  `);
	...
	// Indexes
	db.exec(`CREATE INDEX IF NOT EXISTS idx_tasks_parent ON background_tasks(parent_session_id)`);
	db.exec(`CREATE INDEX IF NOT EXISTS idx_tasks_status ON background_tasks(status)`);
...
export function failTask(taskId: string, error: string): void {
  ...
  `,
	...
	logger.info(`[BackgroundTasks] Task ${taskId.slice(0, 12)}... failed: ${error}`);
}
...
export function getTask(taskId: string): BackgroundTask | null {
	const database = initBackgroundTasks();
	const row = database.prepare("SELECT * FROM background_tasks WHERE id = ?").get(taskId) as
		TaskRow | undefined;
